...
cities_data = ldply(list.files(path="../spain-data/",pattern="csv",full.names=TRUE),.parallel = TRUE,function(filename) {
  # Columns to keep
...
```{r, message=F, warning=F, cache=T}
ggplot(data = cities_data, aes(x = review_scores_accuracy, y = review_scores_rating,color=review_scores_accuracy)) + geom_jitter(size=0.1) + stat_smooth(method = "lm", col = "red")

ggplot(data = cities_data, aes(x = review_scores_cleanliness, y = review_scores_rating,color=review_scores_cleanliness)) + geom_jitter(size=0.1) + stat_smooth(method = "lm", col = "red")

ggplot(data = cities_data, aes(x = review_scores_checkin, y = review_scores_rating,color=review_scores_checkin)) + geom_jitter(size=0.1) + stat_smooth(method = "lm", col = "red")

ggplot(data = cities_data, aes(x = review_scores_communication, y = review_scores_rating,color=review_scores_communication)) + geom_jitter(size=0.1) + stat_smooth(method = "lm", col = "red")

ggplot(data = cities_data, aes(x = review_scores_location, y = review_scores_rating,color=review_scores_location)) + geom_jitter(size=0.1) + stat_smooth(method = "lm", col = "red")
```


```{r, message=F, warning=F, cache=T}
reg <- lm(data=cities_data,log(review_scores_rating)~(review_scores_accuracy+review_scores_cleanliness+review_scores_checkin+review_scores_communication+review_scores_location+review_scores_value+I(review_scores_accuracy^2)+I(review_scores_cleanliness^2)+I(review_scores_checkin^2)+I(review_scores_communication^2)+I(review_scores_location^2)))
summary(reg)
```
