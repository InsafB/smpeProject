...
all_states
usa_data <- completeFun(cities_data, c("price","state"))
usa_data$state <- trim(usa_data$state)
usa_data <- subset(usa_data, usa_data$state != '')
usa_data$state[usa_data$state == 'New York'] <- 'NY'
usa_data$state[usa_data$state == 'ny'] <- 'NY'
usa_data$state[usa_data$state == 'Baja California'] <- 'CA'
usa_data$state[usa_data$state == 'secc Terrazas'] <- 'CA'
usa_data$state[usa_data$state == 'ca'] <- 'CA'
usa_data$state[usa_data$state == 'Ca'] <- 'CA'
usa_data$state[usa_data$state == 'wa'] <- 'WA'
usa_data$state[usa_data$state == 'il'] <- 'IL'
usa_data$state[usa_data$state == 'Il'] <- 'IL'
...
usa_data$state[usa_data$state == 'VT'] <- 'vermont'
...
total <- aggregate(cbind(usa_data$id),
                  by = list(state = usa_data$state),
                  FUN = length)
for (i in 17:49){
  df <- data.frame(paste("state",i),0)
  names(df) <- c("state","V1")
  total <- rbind(df, total)
}
...
data <- merge(total, cnames, by = "state", all.x = TRUE)
nrow(data)
ggplot(total, aes(map_id = total$state)) +
    geom_map(aes(fill = total$V1), map = states_map) +
    scale_fill_continuous("Number of listings", low='coral', high='coral4', guide='colorbar') + 
    expand_limits(x = states_map$long, y = states_map$lat) +
    geom_text(data = data, aes(long, lat, label = data$state), size=2) +
...
```{r, message=F, warning=F, cache=T}
ggplot(data = prices_per_state, aes(x = state, y = price, color = state)) +geom_boxplot(outlier.shape = NA) +theme(axis.text.x = element_text(angle = 90, hjust = 1))+coord_cartesian(ylim = c(0, 750))
```
...
ggplot(data = cities_data_without_null_host_response_time, aes(x = host_response_time, y = price,color=host_response_time)) +geom_boxplot(outlier.shape = NA) +theme(axis.text.x = element_text(angle = 90, hjust = 1))+coord_cartesian(ylim = c(0, 400))
...
ggplot(data = cities_data_without_null_property_type, aes(x = property_type, y = price,color=property_type)) +geom_boxplot(outlier.shape = NA) +theme(axis.text.x = element_text(angle = 90, hjust = 1)) +coord_cartesian(ylim = c(0, 1700))

ggplot(data = cities_data, aes(x = room_type, y = price,color=room_type)) +geom_boxplot(outlier.shape = NA) +theme(axis.text.x = element_text(angle = 90, hjust = 1)) +coord_cartesian(ylim = c(0, 500))
...
ggplot(data = cities_data, aes(x = bed_type, y = price,color=bed_type)) +geom_boxplot(outlier.shape = NA) +coord_cartesian(ylim = c(0, 400))
...
ggplot(data = cities_data, aes(x = instant_bookable, y = price,color=instant_bookable)) +geom_boxplot(outlier.shape = NA) +coord_cartesian(ylim = c(0, 400))

ggplot(data = cities_data, aes(x = cancellation_policy, y = price,color=cancellation_policy)) +geom_boxplot(outlier.shape = NA) +theme(axis.text.x = element_text(angle = 90, hjust = 1)) +coord_cartesian(ylim = c(0, 3000))
```
...
```{r, message=F, warning=F, cache=T}
# Clean the data
price_data <- cities_data
price_data$state <- trim(price_data$state)
price_data <- subset(price_data, price_data$state != '')
price_data$state[price_data$state == 'New York'] <- 'NY'
price_data$state[price_data$state == 'ny'] <- 'NY'
price_data$state[price_data$state == 'Baja California'] <- 'CA'
price_data$state[price_data$state == 'secc Terrazas'] <- 'CA'
price_data$state[price_data$state == 'ca'] <- 'CA'
price_data$state[price_data$state == 'Ca'] <- 'CA'
price_data$state[price_data$state == 'wa'] <- 'WA'
price_data$state[price_data$state == 'il'] <- 'IL'
price_data$state[price_data$state == 'Il'] <- 'IL'
```
...
smp_size <- floor(0.9 * nrow(price_data))
set.seed(357)
train_ind <- sample(seq_len(nrow(price_data)), size = smp_size)
train <- price_data[train_ind, ]
test <- price_data[-train_ind, ]
```
...
For this model, we have a very good value of the adjusted R-Squared (0.822), however, 129346 observations were deleted due to missingness, which leaves us only with 14608 observations.
...
- But a very low value of the adjusted R-squared (0.257).
...
```{r}
price_sub_data <- price_data[price_data$price <= 1000,]
smp_size <- floor(0.9 * nrow(price_sub_data))
set.seed(357)
train_ind <- sample(seq_len(nrow(price_sub_data)), size = smp_size)
train_sub <- price_sub_data[train_ind, ]
test_sub <- price_sub_data[-train_ind, ]
...
summary(p_reg3)
```
Our model is better than the previous one in terms of the adjusted R-squared (0.4386), but it's still not a good one.

In conclusion, there could be many reasons to explain these bad results, for example: 

- Our dataset is not big enough in terms of number of observations.
